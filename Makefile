...
#  boots, and O2C_HELLO_ELF to the o2c-produced demo ELF (Tests/Hello,
#  program 41) to assert its output in the same boots.  Unset by
#  default, so nothing is staged and no machine-specific path is
#  baked into this Makefile.
O2C_ROOT ?=
...
#  until the file server knows the volume (Op_Volume_Info - the volume
#  itself, not a file on it: content is the user's to delete).  The launcher
#  already knows the order, so no program needs to poll for it (the demo's
#  Files.Wait and a poll in the o2c driver were workarounds for this race).
	printf '%s\n' 'await BD0:' >> $(INITRD_ROOT)/System/Manifest
